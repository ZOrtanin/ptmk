...
def addPeoples(arg):
    ...
    if (add_obj.addTableArr(peoples)):
        print('-- успешно --')
...
def getAll() -> None:
    ...
    for person in people:
        human = People({'fio': person.fio, 'birth_date': str(person.birth_date), 'gender': person.gender})
        print(f"ФИО: {human.fio}, Дата рождения: {human.birth_date}, Пол: {human.gender}, кол-во полных лет: {human.age}")
...
def main() -> None:
    ...
    actions.get(arg[1], lambda arg: "Значение по умолчанию")(arg)
